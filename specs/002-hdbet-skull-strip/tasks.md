...
**Purpose**: Docker image definition and package skeleton. No logic.

- [x] T001 Create `docker/hdbet/Dockerfile` — `FROM pytorch/pytorch:2.0.0-cuda11.7-cudnn8-runtime`, `RUN pip install hd-bet==2.0.0`, set `ENTRYPOINT ["hd_bet"]`
...
**Goal**: Given a directory of raw T1/FLAIR NIfTI pairs, run HD-BET in Docker on each and write skull-stripped volumes + brain masks to the output directory. Per-scan timing is logged.

**Independent Test**: Run `pytest tests/unit/preprocessing/test_skull_strip.py -v` and `pytest tests/integration/preprocessing/test_skull_strip_e2e.py::test_smoke_batch -v` — all pass with Docker available.

### Tests for User Story 1 ⚠️ Write and confirm FAILING before T010

- [x] T007 [P] [US1] Write failing unit test for `DockerRunner._build_command()` in `tests/unit/preprocessing/test_skull_strip.py` — asserts GPU flag, volume mounts, pinned image tag, and output path suffix `_bet.nii.gz`
...
- [x] T009 [P] [US1] Write failing unit test for `SkullStripper.process_pair()` in `tests/unit/preprocessing/test_skull_strip.py` — mock `DockerRunner.run()` to succeed; assert `SkullStrippedOutput` fields are populated and `elapsed_seconds` > 0
...
- [x] T011 [US1] Implement `DockerRunner` class in `src/wmh_spark/preprocessing/skull_strip.py` — `_build_command()` constructs `docker run --rm --gpus all -v ... wmh-spark/hdbet:2.0.0` command; `run()` calls subprocess, captures stdout/stderr, raises `DockerExecutionError` on non-zero exit
...
- [x] T013 [US1] Implement `SkullStripper.process_pair()` in `src/wmh_spark/preprocessing/skull_strip.py` — orchestrates Docker run for T1 then FLAIR; derives output paths as `{stem}_bet.nii.gz` / `{stem}_bet_mask.nii.gz`; records elapsed time; returns `SkullStrippedOutput`
...
- [x] T021 [US2] Integrate `QualityGate` into `SkullStripper.process_pair()` in `src/wmh_spark/preprocessing/skull_strip.py` — after Docker run: call `assert_structural()` in smoke-test mode or `check_dsc()` in Phase 2 mode; set `SkullStrippedOutput.status` to `"accepted"` / `"rejected"` / `"error"` accordingly
...
- [x] T027 [P] Implement `__main__` CLI entry point with argparse in `src/wmh_spark/preprocessing/skull_strip.py` — flags: `--input-dir`, `--output-dir`, `--manifest`, `--subject`, `--smoke-test`, `--docker-image`, `--benchmark-out`; exit codes 0/1/2 per `contracts/cli-schema.md`
...
- [x] T032 [P] Write unit test for deterministic output in `tests/unit/preprocessing/test_skull_strip.py` — call `process_pair()` twice on the same input with mocked `DockerRunner.run()`; assert SHA-256 checksums of both output files are identical; covers FR-008 / SC-004 (analysis finding C1)
...
```
T001 (Dockerfile)  ||  T002 (preprocessing/__init__.py)  ||  T003 (test __init__ files)
```
...
```
T007 (DockerRunner command test) [P]
...
```
T011 (DockerRunner) [P]  ||  T012 (_discover_pairs) [P]
    ↓
...
- Constitution Principle II: Docker image tag (`wmh-spark/hdbet:2.0.0`) must never use `latest`
...
- All file paths relative to `wmh-spark/` (inner project directory containing `src/`, `tests/`, `docker/`)
